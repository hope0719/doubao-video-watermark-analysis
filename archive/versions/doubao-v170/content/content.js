...
// ═══════════════════════════════════════════════════════════
  ...
  function isDownloadButton(el) {
    ...
    // v15: 检查祖先是否在登录/导航/工具栏区域（扩大到 8 层，覆盖 chat 页面工具栏）
    let p = el;
    for (let i = 0; i < 8 && p; i++) {
    ...
    // v15: 用直接子节点的纯文本（不含深层后代），避免父级容器继承"保存"二字
    ...
    const isNonDownload =
      ...
      // v15: X 关闭图标直接拒绝
      isCloseIcon ||
      // v15: 容器元素（div/section/header）如果只通过 className 间接匹配，不当作按钮劫持（容器不应被劫持 click）
  ...
  // ═══════════════════════════════════════════════════════════
  // 视频区域「无水印下载」按钮（支持多视频）
  // v15: 改用 fixed 定位 + appendChild 到 body，避免污染视频容器 DOM
  ...
  function addVideoDownloadButtons() {
    ...
    for (const video of videos) {
      ...
      // v15: 改用 fixed 定位 — 初始位置基于 video 的 rect
      ...
      // v15: 附加到 body（不再 appendChild 到视频容器，避免污染 DOM）
  ...
  if (isActivePage()) {
    ...
    // v15: 豆包视频/图片按钮位置更新（fixed 定位需要跟随滚动/resize）
    setInterval(() => {
