(function () {
  ...
  function showNotification(message, type) {
    let notif = document.querySelector(".dw-notif");
    if (!notif) {
    ...
    notif.className = "dw-notif dw-show dw-" + (type || "info");
    clearTimeout(notif._timeout);
    notif._timeout = setTimeout(() => {
      notif.classList.remove("dw-show");
    }, 3000);
  }
  ...
  // ═══════════════════════════════════════════════════════════════
  // v23: 从页面 HTML 提取图片 URL（兜底方案）
  // ═══════════════════════════════════════════════════════════════

  function extractImageUrlsFromPage() {
    // 方法 1: 解析 data-fn-args 中的结构化 JSON
  ...
  function isActivePage() {
    // 修复：匹配所有豆包页面（包括 /chat/123?channel=xxx）
    return /doubao\.com/.test(location.hostname) || /doubao\.com/.test(location.href);
  }
  ...
  async function downloadImage(targetImg, forcedUrl) {
    ...
    showNotification("✅ 无水印图片下载已开始！", "success");
  ...
  const domObserver = new MutationObserver(() => addImageDownloadButtons());
  domObserver.observe(document.body, { childList: true, subtree: true });

  // 页面加载完成后显示通知
  if (isActivePage()) {
    setTimeout(() => showNotification("🫧 去水印自动拦截已开启", "info"), 500);
  }

  console.log("[豆包去水印] v4.3.6 已加载");
})();
