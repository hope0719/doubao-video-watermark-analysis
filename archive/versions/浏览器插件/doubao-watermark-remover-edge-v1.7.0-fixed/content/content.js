...
// ═══════════════════════════════════════════════════════════
  ...
  function associateVidToVideoElements() {
    ...
    // ─── 策略2：DOM 顺序匹配（chat 页主要策略）───
    // chat 页视频元素按生成顺序排列（oldest first）
    // capturedVideoIds 由 unshift 插入（newest first），反转后 = oldest first
    ...
    for (let i = 0; i < unassignedVideos.length && i < unassignedVids.length; i++) {
      ...
      console.log(`[豆包去水印] 主动关联(策略2-DOM顺序): video[${i}] → vid=${unassignedVids[i]}`);
    }
  }
  ...
  function findNearestVideo(element) {
    ...
    for (let i = 0; i < 10 && parent; i++) {
      ...
      if (video) {
        // 找到了包含 video 的容器，返回该容器内的第一个 video
        return video;
      }
  ...
  function findVideoIdForElement(videoEl) {
    ...
    // ─── 兜底：返回 null（不强制使用最近 vid，避免错误匹配）───
    console.log('[豆包去水印] ⚠️ 未匹配到 vid，返回 null');
    return null;
  }
  ...
  function isDownloadButton(el) {
    ...
    for (let i = 0; i < 5 && p; i++) {
      ...
      if (/login|nav|menu|header|sidebar|toolbar|modal|dialog/.test(cls)) return false;
      p = p.parentElement;
    }
    ...
    if (tc && tc.length < 20) {
      const BLACKLISTED_TEXT = ['登录', '注册', '发送', '提交', '取消', '复制', '分享'];
    ...
    const isNonDownload =
      /收藏|关注|点赞|评论|分享|转发|报告|取消保存/i.test(text + title + ariaLabel) ||
