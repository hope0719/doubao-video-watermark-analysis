...
// ═══════════════════════════════════════════════════════════
  ...
  async function extractVideoChatPage(targetVideo) {
    ...
    // v18: 用 targetVideo 元素本身作为 lock key（blob URL 每次可能不同，元素对象才是稳定的）
  ...
  // ═══════════════════════════════════════════════════════════
  // v18: 视频选择面板（多视频时让用户自行选择）
  // ═══════════════════════════════════════════════════════════

  /**
   * 获取页面中所有可下载的视频（视口可见 + 尺寸够大）
   * 返回 [{ video, index, rect, vid, posterUrl }]
   */
  function getDownloadableVideos() {
    ...
    for (let i = 0; i < allVideos.length; i++) {
      ...
      results.push({ video: v, index: i, rect: r, vid, posterUrl });
    }
    return results;
  }
    ...
    for (let i = 0; i < videos.length; i++) {
      ...
      card.innerHTML = `
        ...
        <div class="dw-selector-info">
          <div class="dw-selector-name">视频 ${i + 1}</div>
        ...
        <button class="dw-selector-dl-btn">
  ...
  function getUnassignedAiImages() {
    ...
    for (const img of imgs) {
      ...
      // v18: 排除"非独立 AI 图片"——img 不能是 video/button/a/canvas 元素的子元素或同级 ui 元素
      // 这是修复"视频海报被当成图片"的核心
      ...
      // v18: 进一步——只有满足"AI 生成图特征"（严格 CDN 域名）的图才加按钮
  ...
  function isDownloadButton(el) {
    ...
    // v18: 检查祖先是否在登录/导航/工具栏区域（扩大到 8 层，覆盖 chat 页面工具栏）
    let p = el;
    for (let i = 0; i < 8 && p; i++) {
    ...
    // v18: 用直接子节点的纯文本（不含深层后代），避免父级容器继承"保存"二字
    ...
    const isNonDownload =
      ...
      // v18: X 关闭图标直接拒绝
      isCloseIcon ||
      // v18: 容器元素（div/section/header）如果只通过 className 间接匹配，不当作按钮劫持（容器不应被劫持 click）
  ...
  // ═══════════════════════════════════════════════════════════
  // 视频区域「无水印下载」按钮（支持多视频）
  // v18: 改用 fixed 定位 + appendChild 到 body，避免污染视频容器 DOM
  ...
  function addVideoDownloadButtons() {
    ...
    // v18: 优先只给"视口可见且尺寸够大"的视频加按钮，避免给小缩略图/历史视频加按钮
    const allVideos = document.querySelectorAll('video');
    ...
    for (const video of videos) {
      ...
      // v18: 改用 fixed 定位 — 初始位置基于 video 的 rect
      ...
      const onClick = async (e) => {
        ...
        e.stopPropagation();
        e.stopImmediatePropagation();
        // v18: 点击时强制重新关联 vid（覆盖按钮点击时 vid 可能还没建立）
        try {
          targetVideo.dataset.doubaoVid = ''; // 清空旧的
          const newVid = findVideoIdForElement(targetVideo);
          if (newVid) {
            targetVideo.dataset.doubaoVid = newVid;
            console.log('[豆包去水印] ✅ 覆盖按钮点击时强制关联 vid:', newVid);
          }
        } catch (err) {
          console.log('[豆包去水印] vid 强制关联失败:', err.message);
        }
        await downloadVideo(targetVideo);
      };
      // v18: 只拦截 mousedown/pointerdown/touchstart，避免穿透到原生 UI；
      ...
      // v18: 附加到 body（不再 appendChild 到视频容器，避免污染 DOM）
  ...
  if (isActivePage()) {
    ...
    // v18: 豆包视频/图片按钮位置更新（fixed 定位需要跟随滚动/resize）
    setInterval(() => {
