...
// ═══════════════════════════════════════════════════════════
  ...
  function showVideoSelector() {
    ...
    // v21: ESC 关闭（移除无效的 removeEventListener 死代码）
    const escHandler = (e) => {
      if (e.key === 'Escape') {
        overlay.remove();
  ...
  function getUnassignedAiImages() {
    ...
    for (const img of imgs) {
      ...
      // v21: 排除"非独立 AI 图片"——img 不能是 video/button/a/canvas 元素的子元素或同级 ui 元素
      // 这是修复"视频海报被当成图片"的核心
      ...
      // v21: 进一步——只有满足"AI 生成图特征"（严格 CDN 域名）的图才加按钮
  ...
  function addVideoDownloadButtons() {
    ...
    // v21: 优先只给"视口可见且尺寸够大"的视频加按钮，避免给小缩略图/历史视频加按钮
    const allVideos = document.querySelectorAll('video');
