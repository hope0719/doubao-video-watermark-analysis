...
// ═══════════════════════════════════════════════════════════
  ...
  function isDownloadButton(el) {
    ...
    for (let i = 0; i < 5 && p; i++) {
      ...
      if (/login|nav|menu|header|sidebar|toolbar|modal|dialog|preview|lightbox/.test(cls)) return false;
      p = p.parentElement;
    }
    ...
    if (tc && tc.length < 20) {
      const BLACKLISTED_TEXT = ['登录', '注册', '发送', '提交', '取消', '复制', '分享', '关闭', '全屏', '退出全屏'];
    ...
    const isNonDownload =
      /收藏|关注|点赞|评论|分享|转发|报告|取消保存|关闭|全屏|复制|分享/i.test(text + title + ariaLabel) ||
      /favorite|follow|like|comment|share|report|unsave|close|fullscreen|copy/i.test(title + ariaLabel);
  ...
  document.addEventListener('click', (e) => {
    ...
    for (const element of candidates) {
      ...
      if (elText === '关闭' || elText === '全屏' || elText === '退出全屏') return;
  ...
  // ═══════════════════════════════════════════════════════════
  // 视频区域「无水印下载」按钮（支持多视频）
  // v14: 改用 fixed 定位 + appendChild 到 body，避免污染视频容器 DOM
  ...
  function addVideoDownloadButtons() {
    ...
    for (const video of videos) {
      ...
      // v14: 改用 fixed 定位 — 初始位置基于 video 的 rect
      ...
      // v14: 附加到 body（不再 appendChild 到视频容器，避免污染 DOM）
  ...
  if (isActivePage()) {
    ...
    // v14: 豆包视频/图片按钮位置更新（fixed 定位需要跟随滚动/resize）
    setInterval(() => {
