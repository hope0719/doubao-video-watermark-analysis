...
// ═══════════════════════════════════════════════════════════
  ...
  async function extractVideoChatPage(targetVideo) {
    ...
    // v20: 用元素对象作为 lock key，加 60 秒超时自动清理避免内存泄漏
    ...
    try {
      ...
      // v20: 策略 2：使用拦截到的无水印 URL（仅单视频时安全使用）
  ...
  function showVideoSelector() {
    ...
    header.innerHTML = `
      <div class="dw-selector-header-left">
        <span class="dw-selector-title">🎬 选择要下载的视频（共 ${videos.length} 个）</span>
        ${!isSingle ? `
        <label class="dw-selector-selectall">
          <input type="checkbox" class="dw-selector-selectall-cb" ${isSingle ? 'checked disabled' : ''} />
          <span>全选</span>
        </label>
        ` : ''}
      </div>
    ...
    // 已选视频的 Set（用 video 元素的 dataset 标记）
    const selected = new Set();
    if (isSingle) selected.add(videos[0]);

    for (let i = 0; i < videos.length; i++) {
      const item = videos[i];
      const card = document.createElement('div');
      card.className = 'dw-selector-card' + (isSingle ? ' dw-selector-card-single' : '');
      if (item.visible) card.classList.add('dw-selector-card-visible');
      else card.classList.add('dw-selector-card-hidden');
      ...
      card.innerHTML = `
        <label class="dw-selector-checkbox-wrap">
          <input type="checkbox" class="dw-selector-cb" data-vid-index="${i}" ${isSingle || selected.has(item) ? 'checked' : ''} ${isSingle ? 'disabled' : ''} />
        </label>
      ...
      // 复选框
      const cb = card.querySelector('.dw-selector-cb');
      ...
      card.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.dw-selector-checkbox-wrap')) return;
    ...
    batchBar.className = 'dw-selector-batchbar';
    if (isSingle) {
      batchBar.innerHTML = `<span class="dw-selector-batch-info">💡 点击右侧"下载"按钮下载此视频</span>`;
    } else {
      batchBar.innerHTML = `
        <span class="dw-selector-batch-info">已选 <span class="dw-selector-count">${selected.size}</span> / ${videos.length} 个</span>
        <div class="dw-selector-batch-actions">
          <button class="dw-selector-batch-btn dw-selector-batch-cancel">取消</button>
          <button class="dw-selector-batch-btn dw-selector-batch-confirm" disabled>批量下载</button>
        </div>
      `;
    }

    function updateBatchBar() {
      const info = batchBar.querySelector('.dw-selector-batch-info');
      const confirmBtn = batchBar.querySelector('.dw-selector-batch-confirm');
      if (info) {
        const countEl = info.querySelector('.dw-selector-count');
        if (countEl) countEl.textContent = selected.size;
      }
      if (confirmBtn) {
        confirmBtn.disabled = selected.size === 0;
        confirmBtn.textContent = selected.size > 0 ? `批量下载 (${selected.size})` : '批量下载';
      }
    }

    // 全选/取消全选
    const selectAllCb = header.querySelector('.dw-selector-selectall-cb');
    if (selectAllCb) {
      selectAllCb.addEventListener('change', (e) => {
        const checked = e.target.checked;
        selected.clear();
        list.querySelectorAll('.dw-selector-cb').forEach(cb => {
          cb.checked = checked;
          if (checked) {
            const idx = parseInt(cb.dataset.vidIndex, 10);
            selected.add(videos[idx]);
          }
        });
        updateBatchBar();
      });
    ...
    if (confirmBtn) {
      confirmBtn.addEventListener('click', async () => {
        ...
        for (let i = 0; i < items.length; i++) {
          showNotification(`⏳ 批量下载中 (${i + 1}/${items.length})...`, 'info');
          await downloadVideo(items[i].video);
          // 每个之间间隔 500ms，避免并发竞争
          await new Promise(r => setTimeout(r, 500));
        }
        showNotification(`✅ 批量下载完成，共 ${items.length} 个视频`, 'success');
      });
    }

    // 取消按钮
    const cancelBtn = batchBar.querySelector('.dw-selector-batch-cancel');
    if (cancelBtn) {
    ...
    // v20: ESC 关闭（移除无效的 removeEventListener 死代码）
    const escHandler = (e) => {
      if (e.key === 'Escape') {
        overlay.remove();
  ...
  function getUnassignedAiImages() {
    ...
    for (const img of imgs) {
      ...
      if (src.includes('icon') || src.includes('logo') || src.includes('avatar') ||
          src.includes('favicon') || src.includes('badge') || src.includes('sticker') ||
          src.includes('emoji') || src.includes('headimg') || src.includes('profile')) continue;

      // v20: 排除"非独立 AI 图片"——img 不能是 video/button/a/canvas 元素的子元素或同级 ui 元素
      // 这是修复"视频海报被当成图片"的核心
      const parent = img.parentElement;
      if (!parent) continue;
      const parentTag = (parent.tagName || '').toLowerCase();
      // 视频内部的 poster（HTML5 video 元素的子元素 img 不太常见，但有些页面用 div 模拟）
      if (parentTag === 'video' || parentTag === 'picture') continue;
      // poster 属性专属过滤：如果 img.src 来自 video.poster 同一图，src 经常相同，跳过
      // （用更严格的方法：找出"img 是不是视频封面"）
      let nearestVideo = null;
      let p = parent;
      for (let i = 0; i < 4 && p; i++) {
        // 1) 父级本身就是 video 元素
        if (p.tagName && p.tagName.toLowerCase() === 'video') { nearestVideo = p; break; }
        // 2) 父级是 video 元素直接祖先
        if (p.querySelector && p.querySelector('video')) {
          // 但要确认 video 是直接子级，不只是远房
          const videos = p.querySelectorAll(':scope > video, :scope > div > video, :scope > div > div > video');
          if (videos.length > 0) { nearestVideo = videos[0]; break; }
        }
        p = p.parentElement;
      }
      if (nearestVideo) {
      ...
      // v20: 进一步——只有满足"AI 生成图特征"（严格 CDN 域名）的图才加按钮
      // 这是把 isAiGeneratedImg 的 CDN 校验下沉到查找阶段
  ...
  function isDownloadButton(el) {
    ...
    // v20: 检查祖先是否在登录/导航/工具栏区域（扩大到 8 层，覆盖 chat 页面工具栏）
    let p = el;
    for (let i = 0; i < 8 && p; i++) {
    ...
    // v20: 用直接子节点的纯文本（不含深层后代），避免父级容器继承"保存"二字
    ...
    const isNonDownload =
      ...
      // v20: X 关闭图标直接拒绝
      isCloseIcon ||
      // v20: 容器元素（div/section/header）如果只通过 className 间接匹配，不当作按钮劫持（容器不应被劫持 click）
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      if (isVideoSharingPage()) {
        ...
        // v20: chat 页面 — 多视频时弹出选择面板
        const hasImages = capturedImageUrls.length > 0;
        const downloadableVideos = getDownloadableVideos();
        const hasVideo = downloadableVideos.length > 0;

        if (hasVideo) {
          // 有视频 → 弹出选择面板（单个视频时面板内部自动直接下载）
          showVideoSelector();
        } else if (hasImages) {
          // 没有视频但有图片 → 下载最新图片
  ...
  // ═══════════════════════════════════════════════════════════
  // 视频区域「无水印下载」按钮（支持多视频）
  // v20: 改用 fixed 定位 + appendChild 到 body，避免污染视频容器 DOM
  ...
  function addVideoDownloadButtons() {
    ...
    // v20: 优先只给"视口可见且尺寸够大"的视频加按钮，避免给小缩略图/历史视频加按钮
    const allVideos = document.querySelectorAll('video');
    ...
    for (const video of videos) {
      ...
      // v20: 跳过尺寸过小的视频（缩略图/小预览）
      const initialRect = video.getBoundingClientRect();
      if (initialRect.width < 200 || initialRect.height < 150) continue;
      ...
      // v20: 改用 fixed 定位 — 初始位置基于 video 的 rect
      ...
      const onClick = async (e) => {
        ...
        e.stopPropagation();
        e.stopImmediatePropagation();
        // v20: 只在 vid 未建立时才尝试关联，不清空已有的（清空会导致重新匹配到错的 vid）
      ...
      // v20: 只拦截 mousedown/pointerdown/touchstart，避免穿透到原生 UI；
      ...
      // v20: 附加到 body（不再 appendChild 到视频容器，避免污染 DOM）
  ...
  if (isActivePage()) {
    ...
    // v20: 豆包视频/图片按钮位置更新（fixed 定位需要跟随滚动/resize）
    setInterval(() => {
