(function () {
  ...
  function createWrapperWithButton(img) {
    ...
    // 找到 img 的父元素 — 直接作为按钮容器
    // 不创建新元素，不移动 img
    const parent = img.parentElement;
    if (!parent) return null;

    // 临时把父元素设为 relative（如果还不是 positioned）
    // 用 inline style 标记，方便清理
    if (!parent.dataset.doubaoImgParent) {
      ...
      if (!isPositioned) {
        ...
        // 父元素如果是 inline，absolute 定位会出问题，临时改 inline-block
        if (computed.display === "inline") {
          parent.style.display = "inline-block";
        }
        parent.dataset.doubaoOldPosition = oldPosition;
        parent.dataset.doubaoOldDisplay = oldDisplay;
      }
      parent.dataset.doubaoImgParent = "1";
    }

    // 创建下载按钮（absolute 定位在父元素右下角，叠加在图片上）
    const btn = document.createElement("button");
    btn.className = "dw-img-btn";
    btn.textContent = cleanUrl ? "✓ 原图" : "原图";
    btn.title = cleanUrl ? "点击下载无水印原图" : "正在拦截原图 URL...";

    // 将 URL 存在 btn 的 dataset 上（点击 O(1) 读取）
    btn.dataset.cleanUrl = cleanUrl || "";
    if (cleanUrl) {
    ...
    btn.addEventListener("click", (e) => {
      ...
      downloadDirect(latestUrl || btn.dataset.cleanUrl || null, img);
    });
  ...
  function updatePendingButtons() {
    for (const [img, btn] of imageButtons) {
      if (!btn) continue;
      const cleanUrl = findCleanUrlForImg(img);
      if (cleanUrl && !btn.dataset.cleanUrl) {
        btn.dataset.cleanUrl = cleanUrl;
        img.dataset.doubaoImageUrl = cleanUrl;
        btn.textContent = "✓ 原图";
        btn.title = "点击下载无水印原图";
  ...
  function scanAndAddButtons() {
    ...
    for (const [img, btn] of imageButtons) {
      if (!document.contains(img)) {
        ...
        if (parent && parent.dataset.doubaoImgParent) {
          if (parent.dataset.doubaoOldPosition !== undefined) {
            parent.style.position = parent.dataset.doubaoOldPosition;
          }
          if (parent.dataset.doubaoOldDisplay !== undefined) {
            parent.style.display = parent.dataset.doubaoOldDisplay;
          }
          delete parent.dataset.doubaoImgParent;
        }
        btn?.remove();
