...
// ═══════════════════════════════════════════════════════════
  ...
  async function extractVideoChatPage(targetVideo) {
    ...
    try {
      ...
      // 辅助函数：比较两个 URL 是否匹配（路径部分）
      function urlPathMatch(url1, url2) {
        if (!url1 || !url2) return false;
        const p1 = url1.split('?')[0];
        const p2 = url2.split('?')[0];
        ...
        // 子串匹配
        if (p1.includes(f2) || p2.includes(f1)) return true;
        // hash 匹配
        const h1 = p1.match(/([a-f0-9]{16,})/i)?.[1];
        const h2 = p2.match(/([a-f0-9]{16,})/i)?.[1];
        if (h1 && h2 && h1 === h2) return true;
        return false;
      }

      // ─── 策略 1：并行调所有 vid 的 API + poster 验证 ───
      // v26: 并行调 API（速度提升 3 倍），然后用 poster 匹配
      if (allKnownVids.length > 0) {
        showNotification('🔄 正在通过 API 获取无水印视频...', 'info');

        // v26: 并行调所有 vid 的 API
        const apiPromises = allKnownVids.map(async (vid) => {
          try {
            const res = await fetch(
              'https://www.doubao.com/samantha/media/get_play_info?aid=497858&device_platform=web&language=zh-CN',
              {
            ...
            if (json.code === 0 && json.data) {
              ...
              return { vid, apiUrl, apiPoster, json };
            }
          } catch (err) {
            console.log('[豆包去水印] vid', vid, 'API 失败:', err.message);
          }
          return null;
        });

        const results = await Promise.all(apiPromises);
        const validResults = results.filter(r => r && r.apiUrl);
        console.log('[豆包去水印] 并行 API 返回:', validResults.length, '个有效结果');

        // v26: 单 vid 时直接使用（不需要 poster 验证）
        if (validResults.length === 1) {
          const r = validResults[0];
          if (targetVideo) targetVideo.dataset.doubaoVid = r.vid;
          console.log('[豆包去水印] ✅ 单 vid 直接使用:', r.vid);
          chrome.runtime.sendMessage({
            action: 'download',
            url: r.apiUrl,
            filename: `doubao_${r.vid}.mp4`
          });
          showNotification('✅ 无水印视频下载已开始！', 'success');
          return;
        }

        // v26: 多 vid 时用 poster 匹配
        if (targetPoster) {
          for (const r of validResults) {
            if (r.apiPoster && urlPathMatch(targetPoster, r.apiPoster)) {
              if (targetVideo) targetVideo.dataset.doubaoVid = r.vid;
              console.log('[豆包去水印] ✅ poster 匹配成功, vid:', r.vid);
              chrome.runtime.sendMessage({
                action: 'download',
                url: r.apiUrl,
                filename: `doubao_${r.vid}.mp4`
          ...
          console.log('[豆包去水印] poster 匹配失败，尝试 videoCoverMap');
        }

        // v26: poster 匹配失败，尝试用 videoCoverMap
        for (const r of validResults) {
          const coverEntry = videoCoverMap.find(v => v.vid === r.vid);
          if (coverEntry && coverEntry.coverUrl && targetPoster && urlPathMatch(targetPoster, coverEntry.coverUrl)) {
            if (targetVideo) targetVideo.dataset.doubaoVid = r.vid;
            console.log('[豆包去水印] ✅ videoCoverMap 匹配成功, vid:', r.vid);
            chrome.runtime.sendMessage({
              action: 'download',
              url: r.apiUrl,
              filename: `doubao_${r.vid}.mp4`
        ...
        // v26: 所有匹配都失败，用预关联的 vid（如果有）
        if (associatedVid) {
          const r = validResults.find(v => v.vid === associatedVid);
          if (r) {
            console.log('[豆包去水印] ⚠️ 使用预关联 vid（未通过 poster 验证）:', r.vid);
            chrome.runtime.sendMessage({
              action: 'download',
              url: r.apiUrl,
              filename: `doubao_${r.vid}.mp4`
