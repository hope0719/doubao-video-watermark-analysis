...
// ═══════════════════════════════════════════════════════════
  ...
  function associateVidToVideoElements() {
    ...
    // v25: 策略0：用 video.poster 匹配 cover URL（最可靠的关联方法）
    //    每个 video 元素的 poster 是唯一的 CDN URL，和 API 拦截到的 coverUrl 匹配
    for (const video of videos) {
      if (video.dataset.doubaoVid) continue;
      const poster = video.poster || '';
      if (!poster) continue;
      for (const mapping of videoCoverMap) {
        if (!mapping.coverUrl) continue;
        if (associatedVids.has(mapping.vid)) continue;
        // 比较 URL 的路径部分（去掉域名和查询参数）
        const posterPath = poster.split('?')[0].split('/').pop();
        const coverPath = mapping.coverUrl.split('?')[0].split('/').pop();
        if (posterPath && coverPath && posterPath === coverPath) {
          video.dataset.doubaoVid = mapping.vid;
          associatedVids.add(mapping.vid);
          console.log(`[豆包去水印] 主动关联(策略0-poster匹配): video → vid=${mapping.vid}`);
          break;
        }
        // 也尝试子串匹配（poster 可能是 coverUrl 的缩略图版本）
        if (poster.includes(coverPath) || mapping.coverUrl.includes(posterPath)) {
          video.dataset.doubaoVid = mapping.vid;
          associatedVids.add(mapping.vid);
          console.log(`[豆包去水印] 主动关联(策略0-poster子串匹配): video → vid=${mapping.vid}`);
          break;
        }
      }
    }
  ...
  async function extractVideoChatPage(targetVideo) {
    ...
    try {
      const targetPoster = targetVideo?.poster || '';
      console.log('[豆包去水印] chat 页面下载, video src =', videoSrc?.slice(0, 120));
      console.log('[豆包去水印] video.poster =', targetPoster?.slice(0, 80));
      console.log('[豆包去水印] data-doubaoVid =', targetVideo?.dataset?.doubaoVid);
      ...
      // ─── 策略 1：逐个调 API + poster_url 验证 ───
      // v25: 关键修复——用 API 返回的 poster_url 和 video.poster 做精确匹配
      //    只有 poster 匹配的 vid 才下载，保证下对视频
      //    下载的是 original_media_info.main_url（videoweb.doubao.com 域名，无水印）
      if (allKnownVids.length > 0) {
        showNotification('🔄 正在通过 API 获取无水印视频...', 'info');

        // 辅助函数：比较两个 URL 的路径部分（去掉域名和查询参数）
        function urlPathMatch(url1, url2) {
          if (!url1 || !url2) return false;
          const path1 = url1.split('?')[0].split('/').pop();
          const path2 = url2.split('?')[0].split('/').pop();
          if (!path1 || !path2) return false;
          // 完全匹配
          if (path1 === path2) return true;
          // 子串匹配（处理缩略图版本）
          if (url1.includes(path2) || url2.includes(path1)) return true;
          // 提取 hash 部分（byteimg URL 中的最后一串字符）
          const hash1 = path1.match(/([a-f0-9]{20,})/i)?.[1];
          const hash2 = path2.match(/([a-f0-9]{20,})/i)?.[1];
          if (hash1 && hash2 && hash1 === hash2) return true;
          return false;
        }
          ...
          try {
            ...
            if (json.code === 0 && json.data) {
              const apiUrl = json.data.original_media_info?.main_url ||
                             json.data.media_info?.[0]?.main_url;
              const apiPoster = json.data.poster_url || '';

              if (apiUrl) {
                // v25: 关键验证——API 返回的 poster_url 是否和 video.poster 匹配
                if (targetPoster && apiPoster) {
                  if (urlPathMatch(targetPoster, apiPoster)) {
                    // ✅ 匹配成功！这是当前视频的 vid
                    if (targetVideo) {
                      targetVideo.dataset.doubaoVid = tryVid;
                    }
                    console.log('[豆包去水印] ✅ poster 匹配成功, vid:', tryVid);
                    chrome.runtime.sendMessage({
                      action: 'download',
                      url: apiUrl,
                    ...
                    console.log('[豆包去水印] ❌ poster 不匹配, 跳过 vid:', tryVid);
                    console.log('[豆包去水印]   video.poster:', targetPoster.slice(0, 80));
                    console.log('[豆包去水印]   api.poster: ', apiPoster.slice(0, 80));
                    continue; // 试下一个 vid
                  }
                } else {
                  // 没有 poster 无法验证，但有预关联的 vid 时直接用
                  if (associatedVid === tryVid) {
                    console.log('[豆包去水印] ⚠️ 无 poster 验证，使用预关联 vid:', tryVid);
                    chrome.runtime.sendMessage({
                      action: 'download',
                      url: apiUrl,
                      filename: `doubao_${tryVid}.mp4`
        ...
        console.log('[豆包去水印] 所有 vid 的 API 均未通过 poster 验证');
      }

      // v25: 兜底 1.5——如果所有 vid 都不匹配 poster，但页面只有 1 个 vid，直接用
      if (allKnownVids.length === 1 && targetPoster) {
        const onlyVid = allKnownVids[0];
        console.log('[豆包去水印] 单 vid 兜底，直接使用:', onlyVid);
        try {
          const res = await fetch(
            'https://www.doubao.com/samantha/media/get_play_info?aid=497858&device_platform=web&language=zh-CN',
            {
              ...
              body: JSON.stringify({ key: onlyVid })
            }
          );
          const json = await res.json();
          if (json.code === 0 && json.data) {
            const apiUrl = json.data.original_media_info?.main_url || json.data.media_info?.[0]?.main_url;
            if (apiUrl) {
              if (targetVideo) targetVideo.dataset.doubaoVid = onlyVid;
              chrome.runtime.sendMessage({ action: 'download', url: apiUrl, filename: `doubao_${onlyVid}.mp4` });
              showNotification('✅ 无水印视频下载已开始！', 'success');
              return;
            }
          }
        } catch (err) {
          console.log('[豆包去水印] 单 vid 兜底失败:', err.message);
        }
      }
