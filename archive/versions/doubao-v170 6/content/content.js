...
// ═══════════════════════════════════════════════════════════
  ...
  function getUnassignedAiImages() {
    ...
    for (const img of imgs) {
      ...
      // v22: 严格排除「视频相关」图片
      // 1) img 不能是 video/button/a/canvas/picture 的子元素
      const parent = img.parentElement;
      if (!parent) continue;
      const parentTag = (parent.tagName || '').toLowerCase();
      if (parentTag === 'video' || parentTag === 'picture' ||
          parentTag === 'button' || parentTag === 'a' || parentTag === 'canvas') continue;

      // 2) 向上搜 6 层祖先 + 检查每层 className，看是不是视频卡片容器
      let isInVideoCard = false;
      let p = parent;
      for (let i = 0; i < 6 && p; i++) {
        const cls = (p.className || '').toString().toLowerCase();
        // 视频卡片/播放器相关 className
        if (/video[-_]?(card|message|player|container|wrapper|item|media)|chat[-_]?video|player[-_]?container|media[-_]?player|video[-_]?preview|preview[-_]?video|video[-_]?thumb|player|playwright/i.test(cls)) {
          isInVideoCard = true;
          break;
        }
        // 3) 祖先元素里包含 video 子树（不仅限直接子）
      ...
      if (isInVideoCard) {
        console.log('[豆包去水印] 跳过视频卡片内的预览图:', src.slice(0, 60));
        continue;
      }
