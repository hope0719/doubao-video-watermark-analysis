...
// ═══════════════════════════════════════════════════════════
  ...
  const videoSrcMap = [];
  // v25: 存储 vid → cover_url 映射（用于 poster 精确匹配）
  const videoCoverMap = [];
  // v11: 存储拦截到的无水印图片 URL（最新的在前）
  // v29: 调试日志 — 全链路追踪
  let _logEntries = [];
  function _dbg(...args) {
    const msg = `[豆包去水印] ${args.join(' ')}`;
    console.log(msg);
    _logEntries.push({ t: Date.now(), msg });
    if (_logEntries.length > 500) _logEntries.shift();
  }

  // ═══ v30: 周期性重试 + DOM 监控，修复"API 拦截时 video 元素未渲染"的根因 ═══
  // 当 capturedVideoIds/videoSrcMap/videoCoverMap 有数据但页面 video 元素还没渲染时，
  // associateVidToVideoElements 会因为 videos.length===0 直接退出，之后再也不会被调用。
  // 解决方案：
  // 1) MutationObserver 监控 DOM 变化，检测到新 video 元素立即触发关联
  // 2) 每 500ms 周期性重试（最多 30 秒），覆盖 MutationObserver 可能的遗漏
  // 3) 下载前强制同步调用一次确保关联

  let _reassociateIntervalId = null;
  let _reassociateStartTime = 0;
  const _REASSOCIATE_INTERVAL_MS = 500;
  const _REASSOCIATE_MAX_DURATION = 30000; // 30 秒后停止重试

  /**
   * 启动周期重试：在 video 元素出现前持续尝试关联
   * v30: 解决"API 响应先于 video 元素渲染"的时序问题
   */
  function _startPeriodicReassociate() {
    if (_reassociateIntervalId) return; // 已经在运行
    _reassociateStartTime = Date.now();
    _dbg('启动周期性重试关联（30秒超时）');
    _reassociateIntervalId = setInterval(() => {
      // 超时停止
      if (Date.now() - _reassociateStartTime > _REASSOCIATE_MAX_DURATION) {
        clearInterval(_reassociateIntervalId);
        _reassociateIntervalId = null;
        return;
      }
      associateVidToVideoElements();
    }, _REASSOCIATE_INTERVAL_MS);
  }

  /**
   * 停止周期性重试
   */
  function _stopPeriodicReassociate() {
    if (_reassociateIntervalId) {
      clearInterval(_reassociateIntervalId);
      _reassociateIntervalId = null;
      _dbg('停止周期性重试关联');
    }
  ...
  // v30: MutationObserver 监控 DOM 变化，检测到新 video 元素立即触发关联
  const _videoObserver = new MutationObserver(() => {
    // 延迟 50ms 等 DOM 稳定
    setTimeout(() => {
      const videoCount = document.querySelectorAll('video').length;
      if (videoCount > 0 && (capturedVideoIds.length > 0 || videoSrcMap.length > 0 || videoCoverMap.length > 0)) {
        associateVidToVideoElements();
      }
    }, 50);
  });
  ...
  // v30: 页面加载/显示/获得焦点时触发关联，解决 SPA 导航后 video 元素异步渲染
  ['DOMContentLoaded', 'load', 'pageshow', 'focus', 'visibilitychange'].forEach(evt => {
    window.addEventListener(evt, () => {
      setTimeout(associateVidToVideoElements, 100);
    });
  });

  // v30: 定期检测 video 元素是否出现（兜底，MutationObserver 可能遗漏）
  _startPeriodicReassociate();
  const capturedImageUrls = [];

  // v21: fallback_api 解析出的真·无水印视频 URL（来自 inject.js 的 CHAT_CLEAN_VIDEO 消息）
  const capturedCleanVideoUrls = [];
  ...
  window.addEventListener('message', (event) => {
    ...
    // v25: 接收 vid → cover URL 映射
    if (event.data.type === 'VIDEO_COVER_MAP') {
      const { vid, coverUrl } = event.data.data;
      console.log(`[豆包去水印] 捕获 vid→cover 映射: ${vid} → ${coverUrl?.slice(0, 80)}`);
      if (!videoCoverMap.find(v => v.vid === vid)) {
        videoCoverMap.unshift({ vid, coverUrl, timestamp: event.data.timestamp });
        if (videoCoverMap.length > 20) videoCoverMap.pop();
      }
      // 主动关联
      associateVidToVideoElements();
    }

    // v29: 接收 video.src CDN URL → vid 映射（最可靠的关联方式）
  if (event.data.type === 'VIDEO_SRC_VID') {
    const { vid, cdnUrl } = event.data.data;
    _dbg(`video.src CDN 拦截: vid=${vid}, cdnUrl=${cdnUrl?.slice(0, 80)}`);
    // 直接找到 src 匹配这个 cdnUrl 的 video 元素，写入 vid
    const videos = document.querySelectorAll('video');
    for (const video of videos) {
      if (video.dataset.doubaoVid) continue;
      const src = video.src || video.currentSrc || '';
      if (src === cdnUrl || (cdnUrl && src.includes(cdnUrl))) {
        video.dataset.doubaoVid = vid;
        _dbg(`video.src CDN 直接关联: video → vid=${vid}`);
        break;
      }
    }
  }
  ...
  // v11: 接收图片 URL
    ...
    // v21: 来自 inject.js 的真·无水印视频（fallback_api 解析结果）
    if (event.data.type === 'CHAT_CLEAN_VIDEO') {
      const { url } = event.data.data;
      console.log(`[豆包去水印] 收到真·无水印视频: ${url?.slice(0, 80)}`);
      if (url && !capturedCleanVideoUrls.find(v => v.url === url)) {
        capturedCleanVideoUrls.unshift({ url, timestamp: event.data.timestamp });
        if (capturedCleanVideoUrls.length > 10) capturedCleanVideoUrls.pop();
      }
      // 为页面中的视频补充下载按钮（指向无水印视频）
      setTimeout(addVideoDownloadButtons, 200);
    }
  });
  ...
  function associateVidToVideoElements() {
    ...
    _dbg(`associateVidToVideoElements 开始: ${videos.length} 个 video, ${allVids.length} 个 vid known, ${videoCoverMap.length} 个 cover map`);

    if (allVids.length === 0 && videoCoverMap.length === 0) return;

    // 已关联的 vid 集合（防止重复关联）
    const associatedVids = new Set();

    // ─── 策略0：video.poster 匹配 videoCoverMap 中的 cover URL（最可靠）───
    if (videoCoverMap.length > 0) {
      _dbg(`策略0: 开始 poster 匹配, ${videoCoverMap.length} 个 cover map`);
      for (const entry of videoCoverMap) {
        _dbg(`  cover map: vid=${entry.vid}, cover=${entry.coverUrl?.slice(0, 80)}`);
      }
      for (const video of videos) {
        if (video.dataset.doubaoVid) continue;
        const posterUrl = video.poster || '';
        _dbg(`  检查 video: poster=${posterUrl.slice(0, 80)}`);
        if (!posterUrl) continue;

        for (const entry of videoCoverMap) {
          if (associatedVids.has(entry.vid)) continue;
          if (posterUrl.includes(entry.coverUrl) || entry.coverUrl.includes(posterUrl)) {
            video.dataset.doubaoVid = entry.vid;
            associatedVids.add(entry.vid);
            _dbg(`策略0 poster匹配成功: video → vid=${entry.vid}`);
            break;
          }
        }
      }
    } else {
      _dbg(`策略0: 跳过（videoCoverMap 为空）`);
    }
  ...
  /**
   * 精确判断元素是否真正处于 video 子树上下文内（不含兜底远处视频）。
   * 用于区分"视频保存按钮"与"图片保存按钮"——图片按钮不在 video 子树内，
   * 不应被误判为视频导致走 downloadVideo。
   */
  function isInsideVideoContext(element) {
    if (!element) return false;
    if (element.tagName === 'VIDEO') return true;
    // 1) 最近祖先就是 video
    if (element.closest && element.closest('video')) return true;
    // 2) 祖先链（最多 5 层）的直接子级包含 video（避免命中兄弟节点的远处 video）
    let p = element.parentElement;
    for (let i = 0; i < 5 && p; i++) {
      if (Array.from(p.children).some(c => c.tagName === 'VIDEO')) return true;
      p = p.parentElement;
    }
    return false;
  }
  ...
  function findVideoIdForElement(videoEl) {
    // v20: null 入参也返回 null，保持兜底行为一致
    if (!videoEl) return null;

    // ─── 最优先：直接读 data-doubao-vid 属性 ───
    if (videoEl.dataset.doubaoVid) {
      _dbg(`findVideoIdForElement: 直接命中 data-doubao-vid=${videoEl.dataset.doubaoVid}`);
      return videoEl.dataset.doubaoVid;
    }

    _dbg(`findVideoIdForElement: 需要 fallback, capturedVideoIds=${capturedVideoIds.length} 个, videoSrcMap=${videoSrcMap.length} 个`);
    ...
    if (!isChatPage() && src && !src.startsWith('blob:')) {
      for (const entry of capturedVideoIds) {
        if (entry.id && src.includes(entry.id)) {
          ...
          _dbg(`Fallback1: CDN URL 匹配 vid=${entry.id}`);
          return entry.id;
        }
      }
      for (const mapping of videoSrcMap) {
        if (mapping.vid && src.includes(mapping.vid)) {
          ...
          _dbg(`Fallback1: videoSrcMap 匹配 vid=${mapping.vid}`);
          return mapping.vid;
        }
      }
    }
    ...
    if (src && !src.startsWith('blob:')) {
      ...
      if (vidMatch) {
        ...
        _dbg(`Fallback2: 正则提取 vid=${vidMatch[1]}`);
        return vidMatch[1];
      }
    }

    // v20: Fallback 3：搜索周围 DOM（限制 3 层，避免跨到其他视频卡片）
    _dbg(`Fallback3: 开始搜索周围 DOM, src=${src.slice(0, 100)}`);
      ...
      for (const link of links) {
        ...
        for (const entry of capturedVideoIds) {
          if (entry.id && href.includes(entry.id)) {
            videoEl.dataset.doubaoVid = entry.id;
            _dbg(`Fallback3: 周围 DOM 匹配 vid=${entry.id}, 层级=${i}`);
            return entry.id;
          }
        }
      }
    ...
    // v30: Fallback 3.5：从 videoSrcMap 的 vid 匹配 chat 页面的 poster 包含 vid
    if (videoSrcMap.length > 0) {
      const posterUrl = videoEl.poster || '';
      if (posterUrl) {
        for (const mapping of videoSrcMap) {
          if (mapping.vid && posterUrl.includes(mapping.vid)) {
            videoEl.dataset.doubaoVid = mapping.vid;
            _dbg(`Fallback3.5: poster 包含 vid=${mapping.vid}`);
            return mapping.vid;
          }
        }
      }
    ...
    _dbg(`Fallback4: DOM 顺序匹配, videoIndex=${videoIndex}, 共 ${allVideos.length} 个 video, capturedVideoIds=${capturedVideoIds.length}`);
    if (videoIndex >= 0) {
      ...
      // v30: 让最老的 vid 对应最老的 video（与 DOM 生成顺序一致）
      allVids.reverse(); // newest first → oldest first
      ...
      if (unassignedIndex < allVids.length) {
        ...
        _dbg(`Fallback4: DOM 顺序匹配 vid=${vid}, unassignedIndex=${unassignedIndex}`);
        return vid;
      }
    }

    _dbg(`⚠️ findVideoIdForElement: 所有策略均未匹配，返回 null`);
    return null;
  ...
  async function extractVideoChatPage(targetVideo) {
    ...
    try {
      // v27: 回到 V8 的核心逻辑——用 findVideoIdForElement 拿 vid → 调 API → 下载无水印 URL
      const targetVid = findVideoIdForElement(targetVideo);
      _dbg(`chat 页面下载: videoSrc=${videoSrc?.slice(0, 120)}, targetVid=${targetVid}, data-doubao-vid=${targetVideo?.dataset?.doubaoVid}`);

      // ─── 策略 0（v21 新增）：优先使用 fallback_api 解析出的真·无水印视频 ───
      if (capturedCleanVideoUrls.length > 0) {
        const clean = capturedCleanVideoUrls[0].url;
        _dbg(`策略0: 使用 fallback_api 真·无水印视频=${clean.slice(0, 80)}`);
        showNotification('🔄 正在下载无水印视频...', 'info');
        chrome.runtime.sendMessage({
          action: 'download',
          url: clean,
      ...
      if (targetVid) {
        showNotification('🔄 正在通过 API 获取无水印视频...', 'info');
        _dbg(`策略1: 使用 vid=${targetVid} 调 API`);
        ...
        try {
          ...
          const json = await res.json();
          _dbg(`策略1: API 响应 code=${json.code}, hasData=${!!json.data}`);
          if (json.data) {
            _dbg(`策略1: original_media_info.main_url=${json.data.original_media_info?.main_url?.slice(0, 80)}`);
            _dbg(`策略1: media_info[0].main_url=${json.data.media_info?.[0]?.main_url?.slice(0, 80)}`);
          }
          ...
          if (json.code === 0 && json.data) {
            ...
            if (apiUrl) {
              _dbg(`策略1: 下载无水印 URL=${apiUrl.slice(0, 80)}`);
              chrome.runtime.sendMessage({
                action: 'download',
                url: apiUrl,
          ...
          _dbg(`策略1: API 未返回有效数据，尝试下一策略`);
        } catch (err) {
          _dbg(`策略1: API 调用失败: ${err.message}`);
        }
      } else {
        _dbg(`策略1: 跳过（无 targetVid）`);
      }

      // v27: 策略 2：使用拦截到的无水印 URL（仅单视频时安全使用）
      ...
      // v28: 兜底 — 直接下载 video src（保留与 V8 一致的行为）
      //    虽带水印，但比"获取失败"更友好
      if (videoSrc && videoSrc.startsWith('http')) {
        _dbg(`兜底: 直接下载 video src=${videoSrc.slice(0, 80)}`);
        ...
        _dbg(`兜底: 无视频地址且无 targetVid`);
        showNotification('⏳ 未识别到视频 ID，请等待页面加载完成后重试', 'warning');
      } else {
        _dbg(`兜底: targetVid 存在但策略都失败`);
        showNotification('❌ 获取无水印视频失败，请稍后重试', 'error');
      }
  ...
  function isDownloadButton(el) {
    ...
    // v21: 检查祖先是否在登录/导航/工具栏区域（扩大到 8 层）
    // 注意：不再排除 chat-（对话内容区的图片卡片外壳通常含 chat-，否则图片保存按钮会被误杀）
    let p = el;
    for (let i = 0; i < 8 && p; i++) {
      const cls = (p.className || '').toString().toLowerCase();
      if (/login|nav|menu|header|sidebar|modal|dialog|preview|lightbox/.test(cls)) return false;
      p = p.parentElement;
    }
  ...
  document.addEventListener('click', (e) => {
    ...
    for (const element of candidates) {
      ...
      if (isDownloadButton(element)) {
        ...
        const targetImg = findNearestImage(element);
        if (isInsideVideoContext(element)) {
          const targetVideo = findNearestVideo(element);
          console.log('[豆包去水印] 拦截视频按钮:', element.textContent?.trim()?.slice(0, 20) || 'icon');
          downloadVideo(targetVideo);
        } else if (targetImg) {
          console.log('[豆包去水印] 拦截图片保存按钮:', element.textContent?.trim()?.slice(0, 20) || 'icon');
          downloadImage(targetImg);
        } else {
          console.log('[豆包去水印] 拦截下载按钮(兜底图片):', element.textContent?.trim()?.slice(0, 20) || 'icon');
          downloadImage(null);
        }
        return;
      }
  ...
  function hookDownloadButton(el) {
    ...
    const handler = (e) => {
      ...
      e.stopPropagation();
      e.stopImmediatePropagation();
      const targetImg = findNearestImage(el);
      if (isInsideVideoContext(el)) {
